...
- `.github/workflows/cd-dev.yml` -- (nuevo) `workflow_dispatch`, `environment: dev`, `permissions: {contents: read, id-token: write, packages: write}`: build+push imagen (docker/setup-buildx-action, docker/build-push-action) → `azure/login@v3` (OIDC) → `terraform init` (backend real `stautotfstate`/`tfstate`/`key=dev.tfstate`) → `terraform apply -auto-approve` → build Angular → deploy a Static Web App (`Azure/static-web-apps-deploy@v1`, token enmascarado desde `terraform output`)
...
**Acceptance Criteria:**
- Given el pipeline de CI en verde, when se dispara manualmente `cd-dev.yml` vía `workflow_dispatch`, then se ejecuta `terraform apply` real contra Azure y se aprovisiona (o actualiza) el resource group de dev con los 8 recursos (6 del spine original + Log Analytics Workspace + Container App).
...
### Review Findings (segunda pasada, 2026-08-29 -- revisión cross-LLM 4 capas)

- [x] [Review][Defer] Nada en el workflow verifica que CI esté verde para el SHA desplegado antes de correr `terraform apply` real -- el único guard es `if: github.ref == 'refs/heads/main'` [`.github/workflows/cd-dev.yml:42`]. `epic-1-context.md` fija "Story 1.3 (CI) es prerequisito de Story 1.4 (CD requiere pipeline verde antes de deploy manual)" como dependencia explícita entre historias, pero hoy solo se cumple por convención humana (README dice "normalmente main con CI en verde"). **Decisión (2026-08-29):** el gate correcto es branch protection en `main` exigiendo los checks `backend`/`frontend` -- ya documentado en el README (sección "Branch protection"), no un step nuevo en `cd-dev.yml`. Deferred, razón: ya cubierto por branch protection (bloqueado por plan Free de GitHub -- activar en cuanto el plan lo permita, ver README).
- [x] [Review][Patch] Smoke check no usa `--fail` en el curl -- un 502 del ingress de Azure Container Apps (target_port mal configurado, justo lo que el check dice detectar) cuenta como "alcanzable" igual que un 404 legítimo del scaffold, porque solo verifica que la conexión TCP/TLS se establezca, nunca el status code [`.github/workflows/cd-dev.yml:189`]. **Aplicado:** el check ahora captura el status code (`-w "%{http_code}"`) y falla explícitamente en 502/503/504, tolerando cualquier otra respuesta.
- [x] [Review][Patch] El Static Web App nunca se verifica alcanzable -- el smoke check solo hace curl al FQDN de la API (`cd-dev.yml:177`), nunca a `$swa_host` (`cd-dev.yml:178`), aunque el AC3 de esta historia exige "el sitio carga y las rutas de Angular no devuelven 404" [`.github/workflows/cd-dev.yml:174-191`]. **Aplicado:** el mismo check de status code ahora corre también contra `$swa_host`.
- [x] [Review][Patch] Los dos comandos `terraform plan -var environment=dev` documentados en `infra/terraform/README.md` para validación local ahora fallan porque `api_container_image` es una variable requerida sin default agregada por esta historia [`infra/terraform/README.md:80`, `infra/terraform/README.md:120`]. **Aplicado:** ambos comandos incluyen `-var api_container_image=...`.
- [x] [Review][Patch] El token de deploy del Static Web App no se valida no-vacío antes de enmascarar/inyectar -- si `terraform output -raw static_web_app_api_key` devuelve una cadena vacía (sin error), el fallo aparece más adelante en el deploy de forma confusa en vez de fallar rápido con un mensaje claro [`.github/workflows/cd-dev.yml:129`]. **Aplicado:** chequeo `[ -n "$token" ]` antes de enmascarar/escribir a `$GITHUB_OUTPUT`.
- [x] [Review][Patch] Ruta de build de Angular (`web/dist/web/browser`) hardcodeada dos veces sin chequeo de existencia previo -- falla igual si el path cambia, pero con el mensaje genérico de `cp: No such file or directory` en vez de un error explícito [`.github/workflows/cd-dev.yml:157`, `.github/workflows/cd-dev.yml:165`]. **Aplicado:** `test -d` con mensaje `::error::` explícito antes del `cp`.
- [x] [Review][Patch] El Code Map de este mismo spec dice que el workflow usa `azure/login@v2`, pero el `cd-dev.yml` real usa `@v3` -- el spec ya está desincronizado del código que describe [`_bmad-output/implementation-artifacts/spec-1-4-cd-manual-desplegar-el-esqueleto-a-un-ambiente-de-dev.md:57`]. **Aplicado:** Code Map corregido a `azure/login@v3`.
- [x] [Review][Patch] El primer Acceptance Criteria de este spec dice "7 recursos (5 del spine original + Log Analytics Workspace + Container App)", pero el spine base (commit `3d6b828`, antes de esta historia) ya tenía 6 recursos `azurerm_*`, no 5 -- el total real tras esta historia es 8, no 7 [`_bmad-output/implementation-artifacts/spec-1-4-cd-manual-desplegar-el-esqueleto-a-un-ambiente-de-dev.md:77`]. **Aplicado:** AC corregido a "8 recursos (6 del spine original + Log Analytics Workspace + Container App)".
- [x] [Review][Defer] Sin scan de vulnerabilidades de imagen (Trivy/Grype) entre el build de la imagen de la API y el push a GHCR -- deferred, hardening de supply-chain, mismo bucket que el pinning de Actions/base images ya diferido en la historia 1.3.
- [x] [Review][Defer] `src/Api/Dockerfile` no fija un `USER` no-root explícito -- depende implícitamente del default de la imagen base `aspnet:10.0` -- deferred, hardening de contenedor sin urgencia mientras el scaffold no maneje datos reales.
- [x] [Review][Defer] `dotnet restore` en el Dockerfile corre sin `--locked-mode` ni `packages.lock.json` -- dos builds del mismo commit podrían resolver versiones de paquete NuGet distintas con el tiempo, lo que debilita la garantía de "tag `:sha` inmutable" -- deferred, cambio de configuración de build que corresponde a una historia de CI/dependencias, no a esta de CD.
- [x] [Review][Defer] Sin política de retención/limpieza de tags `:sha` en GHCR -- cada disparo manual publica un tag inmutable nuevo sin expiración, así que el almacenamiento del paquete crece sin límite -- deferred, costo operativo a monitorear, no bloqueante para dev.
- [x] [Review][Defer] Sin runbook de rollback documentado para un deploy malo -- el I/O matrix del spec ya acepta "no hay rollback automático, re-disparar el workflow" pero no da los pasos para revertir a un `:sha` anterior conocido-bueno mientras se prepara un fix -- deferred, documentación operativa.
- [x] [Review][Defer] El nuevo Log Analytics Workspace (`azurerm_log_analytics_workspace.main`) no tiene documentado cómo consultarlo para debug (`az containerapp logs show`, query de ejemplo) -- el recurso existe pero su uso no es descubrible para quien depure un deploy fallido -- deferred, documentación operativa.
- [x] [Review][Defer] `terraform plan` no se guarda como artifact del workflow (`actions/upload-artifact`) -- el único registro de qué se estaba por aplicar contra Azure real vive en los logs efímeros del job -- deferred, trazabilidad/auditoría.
- [x] [Review][Defer] Sin notificación de fallo (Slack/Teams/GitHub issue) cuando el job de deploy falla -- combinado con `cancel-in-progress: false` y el re-disparo manual como único camino de recuperación, un dispatch fallido puede pasar desapercibido hasta que alguien revise la pestaña Actions -- deferred, observabilidad operativa.
- [x] [Review][Defer] `web/staticwebapp.config.json` define `navigationFallback` pero no `globalHeaders` (CSP, `X-Content-Type-Options`, `X-Frame-Options`) -- deferred, hardening razonable de agregar cuando el scaffold tenga contenido/lógica real, no bloqueante para un sitio sin datos todavía.
- [x] [Review][Defer] El `concurrency` group `cd-dev` (`cancel-in-progress: false`) puede descartar silenciosamente un dispatch ya encolado si llega un tercero antes de que termine el primero -- comportamiento estándar de GitHub Actions (solo se conserva el run encolado más reciente), bajo riesgo dado que los deploys son manuales e infrecuentes -- deferred.
